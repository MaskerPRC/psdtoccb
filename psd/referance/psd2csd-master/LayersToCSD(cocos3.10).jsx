...
function run () {
	...
	var json = '<GameFile>\n';
	json += '\t<PropertyGroup Name="' + docName + '" Type="Scene" ID="' + guid() + '" Version="3.10.0.0" />\n';
	json += '\t<Content ctype="GameProjectContent">\n';
	json += '\t\t<Content>\n';
	json += '\t\t<Animation Duration="0" Speed="1.0000" />\n';
	json += '\t\t<ObjectData Name="Scene" ctype="GameNodeObjectData">\n';
	// tagIndex += 1;
	...
	for (var skinName in skins) {
		...
		for (var i = 0; i < skinLayersCount; i++) {
            ...
            json += '\t\t\t\t\t<PrePosition X="0.0000" Y="0.0000" />\n';
            json += '\t\t\t\t\t<PreSize X="0.0000" Y="0.0000" />\n';
            json += '\t\t\t\t\t<FileData Type="Normal" Path="' + relImagesDir + placeholderName + '.png" Plist=""/>\n';
            json += '\t\t\t\t\t<BlendFunc Src="1" Dst="771" />\n';
			json += '\t\t\t\t</AbstractNodeData>\n';
